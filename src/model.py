...
class Vector_BiLSTM_MLP(Module):
    def __init__(self, num_classes: int, data_type: str, hidden_size: int, num_layers: int, mlp_size: int,
        ...
        self.FF2_MLP_hieve = MLP(mlp_output_dim, hieve_mlp_size, 2*proj_output_dim)
        self.FF2_MLP_matres = MLP(mlp_output_dim, matres_mlp_size, 2*proj_output_dim)
    ...
    def forward(self, batch: Tuple[torch.Tensor], device: torch.device, data_type: str):
        ...
        output_A2 = self.FF1(output_A) #[batch_size, 2*proj_output_dim]
        output_B2 = self.FF1(output_B)
        output_C2 = self.FF1(output_C)

        if data_type == "hieve":
            output_A1 = self.FF2_MLP_hieve(output_A1) #[batch_size, mlp_output_dim]
            output_B1 = self.FF2_MLP_hieve(output_B1)
            output_C1 = self.FF2_MLP_hieve(output_C1)

            output_A2 = self.FF2_MLP_hieve(output_A2) #[batch_size, 2*proj_output_dim]
            output_B2 = self.FF2_MLP_hieve(output_B2)
            output_C2 = self.FF2_MLP_hieve(output_C2)
            
            
            dot_A_B = torch.sum(torch.mul(output_A1, output_B1),1).unsqueeze(-1) # [batch_size, 2] or [batch_size, 1]
            dot_B_C = torch.sum(torch.mul(output_B1, output_C1),1).unsqueeze(-1)
            dot_A_C = torch.sum(torch.mul(output_A1, output_C1),1).unsqueeze(-1)
            
            dot_B_A = torch.sum(torch.mul(output_B2, output_A2),1).unsqueeze(-1)
            dot_C_B = torch.sum(torch.mul(output_C2, output_B2),1).unsqueeze(-1)
            dot_C_A = torch.sum(torch.mul(output_C2, output_A2),1).unsqueeze(-1)


        elif data_type == "matres":
            output_A1 = self.FF2_MLP_matres(output_A1) #[batch_size, mlp_output_dim]
            output_B1 = self.FF2_MLP_matres(output_B1)
            output_C1 = self.FF2_MLP_matres(output_C1)

            output_A2 = self.FF2_MLP_matres(output_A2) #[batch_size, 2*proj_output_dim]
            output_B2 = self.FF2_MLP_matres(output_B2)
            output_C2 = self.FF2_MLP_matres(output_C2)

            dot_A_B = torch.sum(torch.mul(output_A1, output_B1),1).unsqueeze(-1) #  [batch_size, 1]
            dot_B_C = torch.sum(torch.mul(output_B1, output_C1),1).unsqueeze(-1)
            dot_A_C = torch.sum(torch.mul(output_A1, output_C1),1).unsqueeze(-1)
            ...
            #Hieve=================================================================
            output_A1_hieve = self.FF2_MLP_hieve(output_A1) #[batch_size, mlp_output_dim]
            output_B1_hieve = self.FF2_MLP_hieve(output_B1)
            output_C1_hieve = self.FF2_MLP_hieve(output_C1)
            
            output_A2_hieve = self.FF2_MLP_hieve(output_A2) #[batch_size, 2*proj_output_dim]
            output_B2_hieve = self.FF2_MLP_hieve(output_B2)
            output_C2_hieve = self.FF2_MLP_hieve(output_C2)

            dot_A_B_hieve = torch.sum(torch.mul(output_A1_hieve, output_B1_hieve),1).unsqueeze(-1) #  [batch_size, 1]
            dot_B_C_hieve = torch.sum(torch.mul(output_B1_hieve, output_C1_hieve),1).unsqueeze(-1)
            dot_A_C_hieve = torch.sum(torch.mul(output_A1_hieve, output_C1_hieve),1).unsqueeze(-1)
            
            dot_B_A_hieve = torch.sum(torch.mul(output_B2_hieve, output_A2_hieve),1).unsqueeze(-1)
            dot_C_B_hieve = torch.sum(torch.mul(output_C2_hieve, output_B2_hieve),1).unsqueeze(-1)
            dot_C_A_hieve = torch.sum(torch.mul(output_C2_hieve, output_A2_hieve),1).unsqueeze(-1)
            #=========================================================================

            #Matres===================================================================
            output_A1_matres = self.FF2_MLP_matres(output_A1) #[batch_size, mlp_output_dim]
            output_B1_matres = self.FF2_MLP_matres(output_B1)
            output_C1_matres = self.FF2_MLP_matres(output_C1)
           
            output_A2_matres = self.FF2_MLP_matres(output_A2) #[batch_size, 2*proj_output_dim]
            output_B2_matres = self.FF2_MLP_matres(output_B2)
            output_C2_matres = self.FF2_MLP_matres(output_C2)

            dot_A_B_matres = torch.sum(torch.mul(output_A1_matres, output_B1_matres),1).unsqueeze(-1) #  [batch_size, 1]
            dot_B_C_matres = torch.sum(torch.mul(output_B1_matres, output_C1_matres),1).unsqueeze(-1)
            dot_A_C_matres = torch.sum(torch.mul(output_A1_matres, output_C1_matres),1).unsqueeze(-1)
            
            dot_B_A_matres = torch.sum(torch.mul(output_B2_matres, output_A2_matres),1).unsqueeze(-1)
            dot_C_B_matres = torch.sum(torch.mul(output_C2_matres, output_B2_matres),1).unsqueeze(-1)
            dot_C_A_matres = torch.sum(torch.mul(output_C2_matres, output_A2_matres),1).unsqueeze(-1)
            #==========================================================================

            
            #Concatenating Hieve and Matres============================================
            dot_A_B = torch.cat((dot_A_B_hieve, dot_A_B_matres), 1)  #[batch_size, 2]
            dot_B_C = torch.cat((dot_B_C_hieve, dot_B_C_matres), 1)  
            dot_A_C = torch.cat((dot_A_C_hieve, dot_A_C_matres), 1)
            
            dot_B_A = torch.cat((dot_B_A_hieve, dot_B_A_matres), 1)
            dot_C_B = torch.cat((dot_C_B_hieve, dot_C_B_matres), 1)
            dot_C_A = torch.cat((dot_C_A_hieve, dot_C_A_matres), 1)
            #==========================================================================
