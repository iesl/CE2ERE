    ...
    if data_type == "hieve":
        ...
        if model_type == "box":
            if "10" not in result_dict.keys():
                ...
                metrics[f"[{eval_type}-HiEve] Precision"] = (P_PC+P_CP+P_CR)/3
                metrics[f"[{eval_type}-HiEve] Recall"] = (R_PC+R_CP+R_CR)/3
                metrics[f"[{eval_type}-HiEve] F1 Score"] = (F1_PC+F1_CP+F1_CR)/3
        else:
