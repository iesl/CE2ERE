...
def metric(data_type, eval_type, train_type, y_true, y_pred):
    """
    confusion matrix ex)
    y_true = [2, 0, 2, 2, 0, 1]
    y_pred = [0, 0, 2, 2, 0, 2]
    ...
    if data_type == "hieve":
        ...
        if "0" not in result_dict.keys() or "1" not in result_dict.keys():
            metrics[f"[{eval_type}-HiEve] Precision"] = 0
            metrics[f"[{eval_type}-HiEve] Recall"] = 0
            metrics[f"[{eval_type}-HiEve] F1-PC"] = 0
            metrics[f"[{eval_type}-HiEve] F1-CP"] = 0
            metrics[f"[{eval_type}-HiEve] F1-PC-CP-AVG"] = 0
            return metrics
        else:
